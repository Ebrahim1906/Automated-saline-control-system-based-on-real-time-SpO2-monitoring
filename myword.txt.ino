...
void loop() {
  ...
  // Update SpO2 and salineVolume when beat is detected
  if (checkForBeat(irValue)) {
    SpO2 = calculateSpO2();
    salineVolume = getSalineVolume(SpO2);
  }

  // Send to Blynk every second
  if (millis() - tsLastReport > REPORTING_PERIOD_MS) {
    ...
    Blynk.virtualWrite(V4, salineVolume);
  }
}
...
// Saline volume based on SpO2
float getSalineVolume(float spo2) {
  if (spo2 < 80) {
    return 700.0; // Severe hypoxemia
  } else if (spo2 <= 85) {
    return 600.0; // Moderate hypoxemia
  } else if (spo2 <= 88) {
    return 500.0; // Mild hypoxemia
  } else if (spo2 <= 92) {
    return 400.0; // Mildly low SpO2
  } else if (spo2 <= 95) {
    return 300.0; // Close to normal
  } else {
    return 0.0;   // Normal, no saline needed
  }
}
