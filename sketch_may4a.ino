...
void loop() {
  ...
  // If no hand is placed on the sensor
  if (irValue < 10000) {
    SpO2 = 0;
    salineVolume = 0;

    // Display "Please place your hand" message
    Blynk.virtualWrite(V6, "Please place your hand");

    // Send 0 values to Blynk app
    Blynk.virtualWrite(V3, SpO2);  // Update SpO2 on Blynk
    Blynk.virtualWrite(V4, salineVolume);  // Update saline volume on Blynk

    // Turn off physical LED and Blynk LED
  ...
  // If a hand is placed on the sensor, calculate SpO₂ and saline volume
  SpO2 = calculateSpO2();
